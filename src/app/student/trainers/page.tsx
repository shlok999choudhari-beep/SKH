      ...
      <div className={styles.content}>
        ...
        <main className={styles.main}>
          ...
          <div style={{ display: 'flex', gap: '1rem', borderBottom: '1px solid var(--border)', marginBottom: '1.5rem', paddingBottom: '0.5rem' }}>
            ...
            >
              🔍 Browse Available Trainers ({trainers.length})
            </button>
            <button
            ...
            >
              📅 My Booked Sessions ({mySessions.filter(s => s.status !== 'cancelled').length})
            </button>
          </div>
                          ...
                          <div style={{ display: 'flex', alignItems: 'center', gap: '1rem', marginBottom: '1rem' }}>
                            ...
                            <div style={{ flex: 1, minWidth: 0 }}>
                              ...
                              <div style={{ display: 'flex', alignItems: 'center', gap: '6px', marginTop: '4px' }}>
                                <span style={{ color: '#f59e0b', fontSize: '0.875rem', fontWeight: 600 }}>★ {trainer.rating || 4.9}</span>
                            ...
                            <div style={{ marginBottom: '0.875rem' }}>
                              ...
                              <div style={{ display: 'flex', flexWrap: 'wrap', gap: '0.375rem' }}>
                                {specialtiesList.map((tag: string) => (
                                  <span key={tag} className="badge badge-purple" style={{ fontSize: '0.725rem', padding: '3px 8px' }}>
                                    ⚡ {tag}
                                  </span>
                                ))}
                              </div>
                      ...
                      <div>
                        ...
                        <div style={{ background: 'var(--bg-primary)', padding: '10px 12px', borderRadius: '8px', border: '1px solid var(--border)', marginBottom: '1rem' }}>
                          <p style={{ fontSize: '0.85rem', fontWeight: 600, color: 'var(--text-primary)', marginBottom: '4px' }}>
                            📝 {session.notes || '1-on-1 Mentorship'}
                          </p>
                          <p style={{ fontSize: '0.8rem', color: 'var(--text-secondary)' }}>
                            🕒 {new Date(session.startTime).toLocaleDateString(undefined, { weekday: 'short', month: 'short', day: 'numeric' })} at {new Date(session.startTime).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })} - {new Date(session.endTime).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}
                          </p>
                        </div>
                      </div>
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.65)', backdropFilter: 'blur(4px)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000, padding: '1rem' }}>
          <div style={{ background: 'var(--bg-secondary)', width: '100%', maxWidth: '560px', borderRadius: '16px', border: '1px solid var(--border)', overflow: 'hidden', boxShadow: '0 20px 40px rgba(0,0,0,0.3)' }}>
            <div style={{ padding: '1.25rem 1.5rem', borderBottom: '1px solid var(--border)', display: 'flex', justifyContent: 'space-between', alignItems: 'center', background: 'var(--bg-primary)' }}>
              ...
              <button onClick={() => setSelectedTrainer(null)} style={{ background: 'none', border: 'none', fontSize: '1.25rem', cursor: 'pointer', color: 'var(--text-secondary)' }}>
                ✕
              </button>
            </div>
